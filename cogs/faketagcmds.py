...
class FakeTagCmds(commands.Cog, name="Fake Tag Database Commands",description="Access The Fake Tag Database, New Light's in-house fake tagged ship tracker."):
    ...
    async def submitFakeTagger(self,ctx,hexcode,actualClansTag,screenshot: discord.Attachment,*,shipName):
        if str(ctx.message.author.id) not in banned:
            try:
                ...
                e.timestamp=datetime.datetime.now()
                await ctx.send(embed=e)
                report={"submissionId":id,"hexcode":hexcode,"shipName":shipName,"screenshot":screenshot.url,"submitterName":ctx.message.author.name,"submitterId":ctx.message.author.id,"actualClansTag":actualClansTag,"date":date,"owner":None,"captains":[],"dateOfLastUpdate":date,"lastUpdatedBy":None,"formerShipNames":[]}
    ...
    @commands.command(name="scanFakeTaggedDB",aliases=["sftdb","searchft"],brief="!!CASE SENSITIVE!! Searches for the submission with the corresponding key:value pair.", help="Searches for the submission with the corresponding key:value pair. Valid Keys: submissionId, hexcode, shipName, actualClansTag, submissionDate & dateOfLastUpdate (EST, Format: MM-DD-YYYY; Day & Month can be single digits, year must be all four), submitterName (Name of user who submitted the ship), submitterId (ID of user who submitted the ship), screenshot (Value MUST Be A Discord Attachment URL)")
